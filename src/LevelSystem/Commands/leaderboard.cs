namespace LevelSystem.Commands
{
    ...
    public class leaderboard : ICommand
    {
        ...
        {
            var sorted = Plugin.Singleton.db.GetCollection<PlayerLog>("Players").FindAll().OrderByDescending(o => o.XP).OrderByDescending(o => o.LVL);
            var players = sorted.Take(amount);
            string str = "";
            int index = 1;
            foreach (var log in players)
            {
                str += $"{index}. {log.Nickname} {(log.DNT ? "[УДАЛЕНО]" : $"({log.ID})")}: {log.LVL} уровень, {log.XP}/{Plugin.Singleton.Config.XPPerLevel + (Plugin.Singleton.Config.XPPerNewLevel * log.LVL)} опыта..\n";
                index++;
            }
            return str;
        }
